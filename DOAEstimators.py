...
def ESPRIT_Polarization(signal, model_order, separation=1 / 2):
    ...
    phi_polarization = \
        np.linalg.lstsq(Qs[np.arange(1, antennas * 2, 2), :], Qs[np.arange(0, antennas * 2, 2), :], rcond=None)[0]
...
def SAGE_Polarization(received_signal, model_order, resolution=0.1, separation=1 / 2, E_ref=1):
    ...
    for iter in range(30):
        ...
        for signal in range(model_order):
            ...
            Kx = Ks[signal, signal] * steering_est[:, [signal]] @ np.conj(steering_est[:, [signal]]).T + np.identity(
                N * 2)
            ...
            Ks[signal, signal] = np.abs(((1 / (np.conj(A_filter).T @ A_filter))) @ (
                    (np.conj(A_filter).T @ Cx @ A_filter) / (np.conj(A_filter).T @ A_filter) - 1 / model_order))
